...
class Solution:
    def replaceElements(self, arr: List[int]) -> List[int]:
        ...
        n = len(arr)
        ans = [0] * n
        for i in range(n - 1, -1, -1): # iterate backwards through the list starting at the second last index, ending at the start
            ans[i] = max_num # sets the last value to -1
            max_num = max(max_num, arr[i])
        return ans

        # note, in this case traversing right to left is much better vs left to right
        # also can't update the maximum before storing the result, or else you accidentally use the current value to replace itself
