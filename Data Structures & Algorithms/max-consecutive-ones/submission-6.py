...
# test case with no 1s
# test case ending on a 0
# test case ending on a 1
# test case where all 1s

# space complexity: O(1), only constants, no additional memory.
...
class Solution: 
    def findMaxConsecutiveOnes(self, nums: List[int]) -> int:
        ...
        #         ones_counter += 1
        #         # need to increment if we end on a 1, will miss this case
        ...
        result = counter = 0
        for num in nums:
            counter = counter + 1 if num else 0 # reset to 0 if break the chain
