class Solution:
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        # time is O(n^2) since iterate through constant times through sudoku board, where n is the number of rows in the grid (n*n for every square)
        # space is O(n) only ever keep one set for the current column/ row we are iterating through, where length is max length of the row (possible digits)
