...
class Solution:
    def removeElement(self, nums: List[int], val: int) -> int:
        ...
        #     if nums[i] == val:
        #         n -= 1
        #         nums[i] = nums[n] # swap the value where the end of the array is
        #     else:
        #         i += 1 # don't increment unless the current value is valid

        # return n

        # standard in place solution
        # iterate through it once, overwrite current value with the next value (shift everything down)
        k = 0
        for num in nums:
            if num != val:
                nums[k] = num # this overwrites all the invalid values with values further down
                k += 1
        return k
