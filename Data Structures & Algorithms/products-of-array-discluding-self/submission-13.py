class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        # O(n) time and space, since pass through nums twice and then iterate another time to create result. Create intermediate arrays of length n

        # optimize by not storing prefix and suffix, compute the result as we iterate (two passes, forward and backward)
        n = len(nums)
        # initialize as 1, no impact on multiplication
        ...
        for i in range(1, n):
            prefix_product_up_to_i = nums[i - 1] * prefix_product_up_to_i
            output[i] *= prefix_product_up_to_i
        
        for i in range(n - 2, -1, -1): # go backwards
            suffix_product_up_to_i = nums[i + 1] * suffix_product_up_to_i # we want the product to be product of everything not including the current i
