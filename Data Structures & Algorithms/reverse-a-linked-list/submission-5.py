...
class Solution:
    ...
    # recursive solution # time O(n) since iterate through everything once (recursion, unwinding), space is O(n) since we build stack frames for O(n) recursive calls e.g. hold each sublist in memory
