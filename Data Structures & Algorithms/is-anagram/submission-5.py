...
# it's not an anagram if not all of the letters are used e.g. return false if the length of the strings don't match
...
class Solution:
    def isAnagram(self, s: str, t: str) -> bool: # O(n + m) since iterate through both strings, space complexity is O(1) since at most 26 characters
        if len(s) != len(t): # make sure to compare the lengths first
            return False
